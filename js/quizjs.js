 $(document).ready(function() {
		...
		$('.social li').hide();
		$('.general li').hide();
		$('#MATHS:input').attr('disabled', true);

	function mathsfunction(){
		var mathsans=[];
		var check1ans=[];
		var count,counter;
		var num = $('.mscore_box').length;
			function m1ans(){
...
$('.mainquiz').find('a').removeClass("disabled");
						}
					else
						{
			...
			function mathstimer(){
				...
				function timer(){
					...
					if(count<30)
					{
						$('.mainquiz').not('active').find('a').addClass("disabled");
					}
					if(count == 0)
					{
						clearInterval(counter);				
		...
		var scount,scounter;
			...
			function sciencetimer()
			{
				...
				function tim()
				{
					...
					document.getElementById("sc-timer").innerHTML=scount+"s";
	...
	function socialfunction()
	{
		...
		var socount,socounter;
			...
			function socialtimer(){
				...
				socount =20;
				socounter=setInterval(timer,1000);
				function timer(){
					socount-=1;
					if(socount<20)
					{
...
$('.mainquiz').find('a').removeClass("disabled");
					...
					document.getElementById("s-timer").innerHTML=socount+"s";
